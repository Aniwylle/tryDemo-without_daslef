...
def init_db():
    with get_db() as conn:
        ...
        review text);
        """)
 # крутая забыла добавить в айдишники autoincrement
init_db()
...
def post_admin_change_status(request: Request,
    ...
    if not user_id:
        return RedirectResponse(url="/login", status_code=302)
    with get_db() as conn:
        conn.execute("update requests set status = ? where id = ?", (status, request_id)) # не дописала where id = ?
    return RedirectResponse(url="/admin", status_code=302)
...
def post_add_review(request: Request,
    ...
    with get_db() as conn:
        cur = conn.execute("select * from requests where status = 'Обучение завершено' and id = ? and review is null", (request_id,)).fetchone() #еще не выучила, пришлось подсмотреть
        if cur:
            conn.execute("update requests set review = ? where id = ?", (review, request_id)) #еще не выучила, пришлось подсмотреть и списала неправильно :D вместо set review написала set status
    return RedirectResponse(url="/profile", status_code=302)