...
app.add_middleware(SessionMiddleware, secret_key="bananchiki")
...
def post_add_review(request: Request,
    ...
    if not user_id:
        return RedirectResponse(url="/login", status_code=302)   
    with get_db() as conn:
        cur = conn.execute("select * from requests where status == 'Обучение завершено' and id = ? and review is null", (request_id,)).fetchone() # забыла and id = ? and
        if cur:
