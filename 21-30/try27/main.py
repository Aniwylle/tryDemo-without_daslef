...
app.add_middleware(SessionMiddleware, secret_key="pgADMIN")
...
def post_register(
    ...
    if not ree.fullmatch(r"[a-zA-Z0-9]{6,}", login):
        ...
        with get_db() as conn:
            ...
            if not user:
                cur.execute(
                    "insert into users (login, password, fio, phone, email) values (%s,%s,%s,%s,%s)",
...
@app.post("/login")
def post_login(request: Request, login: str = Form(...), password: str = Form(...)):
    if login == "Admin":
        if password == "KorokNET":
            ...
            return templates.TemplateResponse(
                request=request, name="login.html", context={"error": "Неверные данные"}
            )
...
def get_profile(request: Request):
    ...
    return templates.TemplateResponse(
        request=request, name="profile.html", context={"requests": request_list}
    )


@app.post("/add_review")
def post_add_review(
    request: Request, review: str = Form(...), request_id: str = Form(...)
):
    user_id = request.session.get("user_id")
    if not user_id:
        return RedirectResponse(url="/login", status_code=302)
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "update requests set review = %s where id = %s", (review, request_id)
        )
        conn.commit()
    return RedirectResponse(url="/profile", status_code=302)
...
def get_create_request(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        context={"courses": COURSES, "payments": PAYMENTS},
    )
...
def post_create_request(
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "insert into requests (course_name, date_start, payment_method, user_id) values (%s, %s, %s, %s)",
            (course, date, payment, user_id),
        )
        conn.commit()
    return RedirectResponse(url="/profile", status_code=302)
...
def get_admin(request: Request, status: str | None = Query(None)):
    ...
    return templates.TemplateResponse(
        request=request,
        name="admin.html",
        context={"requests": request_list, "cur_status": status or "Все"},
    )


@app.post("/admin/change_status")
def post_admin_change_status(
    request: Request, status: str = Form(...), request_id: str = Form(...)
):
    is_admin = request.session.get("admin")
    if not is_admin:
        return RedirectResponse(url="/login", status_code=302)
