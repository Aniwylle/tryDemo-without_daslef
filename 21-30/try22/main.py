...
app.add_middleware(SessionMiddleware, secret_key = "filter")
...
def post_register(request: Request,
    ...
    if not ree.fullmatch(r'[a-zA-Z0-9]{6,}', login):
        error = "Логин: Латиница + цифры, не меньше 6 символов"
    elif len(password) <8:
    ...
    return template.TemplateResponse("register.html", {"request": request, "error": error, "login": login, "password": password, "fio": fio, "phone": phone, "email": email})
...
def get_admin(request: Request,
    ...
    with get_db() as conn:
        request_list = conn.execute(base, params).fetchall()
    return template.TemplateResponse("admin.html", {"request": request, "requests": request_list, "cur_status": status or 'Все'})
