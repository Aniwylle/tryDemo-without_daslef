...
app.add_middleware(SessionMiddleware, secret_key="Belarus")
...
def get_admin(request: Request,
    ...
    if not user_id:
        return RedirectResponse(url="/login", status_code=302)
    base = "select requests.*, users.login, users.fio from requests join users on users.id = requests.user_id"
    params = []
...
def check_db():
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("SELECT id, course_name, user_id, date_start FROM requests ORDER BY id")
        all_requests = cur.fetchall()
        
        result = []
        for r in all_requests:
            result.append(f"ID={r['id']}, course={r['course_name']}, user_id={r['user_id']}, date={r['date_start']}")
        
        return {"count": len(all_requests), "requests": result}