...
def post_create_request(request: Request,
    ...
    if not user_id:
        return RedirectResponse(url="/login", status_code=302)
    with get_db() as conn:
        conn.execute("insert into requests(user_id, course_name, date_start, payment_method) values (?,?,?,?)", (user_id, course, date, payment))
...
def get_admin(request: Request):
    ...
    if not user_id:
        return RedirectResponse(url="/login", status_code=302)
    with get_db() as conn:
        request_list = conn.execute("select requests.*, users.login, users.fio from requests, users where users.id = requests.user_id").fetchall()
