...
def get_admin(request: Request):
    ...
    with get_db() as conn:
        request_list = conn.execute("select requests.*, users.login, users.fio from requests, users where user_id = requests.user_id") # запятую забыла между users.login и users.fio
...
def post_admin_change_status(request: Request,
    ...
    with get_db() as conn:
        conn.execute("update requests set status = ? where id = ?", (status, request_id)) #написала (request_id, status)
    return RedirectResponse(url="/admin", status_code=302)
