...
app.add_middleware(SessionMiddleware, secret_key="mewowowow")
...
def get_profile(request: Request):
    ...
    return template.TemplateResponse(
        "profile.html", {"request": request, "requests": request_list}
    )
...
def get_create_request(request: Request):
    ...
    return template.TemplateResponse(
        "create_request.html",
        {"request": request, "courses": COURSES, "payments": PAYMENTS},
    )
...
def post_create_request(
    ...
    with get_db() as conn:
        conn.execute(
            "insert into requests (course_name, date_start, payment_method, user_id) values (?,?,?,?)",
...
def get_admin(request: Request):
    ...
    with get_db() as conn:
        request_list = conn.execute(
            "select requests.*, users.login, users.fio from requests, users where user_id = requests.user_id"
        )
    return template.TemplateResponse(
        "admin.html", {"request": request, "requests": request_list}
    )
...
def post_add_review(
    ...
    with get_db() as conn:
        cur = conn.execute(
            "select * from requests where status = 'Обучение завершено' and id = ? and review is null",
            (request_id,),
        ).fetchone()
        if cur:
            conn.execute(
                "update requests set review = ? where id = ?", (review, request_id)
            )
    return RedirectResponse(url="/profile", status_code=302)
