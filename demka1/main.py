...
def post_register(
    ...
    return templates.TemplateResponse(
        "register.html", {"request": request, "error": "Ошибка"}
    )
...
def post_login(request: Request, login: str = Form(...), password: str = Form(...)):
    ...
    with get_db() as connect:
        ...
        if user and user["password"] == password:
            ...
            return templates.TemplateResponse(
                "login.html", {"request": request, "error": "Неверный пароль или логин"}
            )
...
def create_request(
    ...
    with get_db() as connect:
        ...
        connect.execute(
            "insert into requests (user_id,course_name,date_start,payment_method) values (?,?,?,?)",
            (user_id, courses, date, payment)
        )
        return RedirectResponse(url="/profile", status_code=302)

    return templates.TemplateResponse(
        "create_request.html",
        {"request": request, "courses": COURSES, "payment": PAYMENTS, "error": error},
    )
...
def get_admin(request: Request):
    ...
    with get_db() as connect:
        request_list = connect.execute(
            "select r.*, u.login, u.fio from requests r join users u on r.user_id = u.id order by r.id desc"
        ).fetchall()
        return templates.TemplateResponse(
